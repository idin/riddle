...
# folder is a directory that acts like a dictionary, objects are saved to a directory and retrieved from it
class Folder:
	...
	def __contains__(self, item):
		return self._get_path(key=item, method='pickle').exists() or self._get_path(key=item, method='dill').exists()
	...
	def __setitem__(self, key, value):
		...
		try:
			first_path.save(obj=value, method=method)
		except PicklingError:
			if first_path.exists():
				first_path.delete()
			method = methods[1]
			self._get_path(key=key, method=method).save(obj=value, method=method)
	...
	def __getitem__(self, item):
		...
		return the_path.load(method=method)
